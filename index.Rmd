...
ggplot(covid_g2, aes(x= fecha, y = positivos_nuevos)) +
  geom_histogram(stat='identity', color= "gainsboro") +
...
ggplot(
  ...
  geom_col(position = "dodge") +
  coord_flip()  +
  ggtitle("Cantidad de hombres y mujeres fallecidos por covid en Costa Rica, por mes.") +
  xlab("Mes del año") +
  ylab("Fallecidos") +
...
ggplot() +
  geom_col(data= covid_h, aes( x=mes, y= sum_h), color= "blue", width = 0.45,
           position = position_nudge(x = -0.225)) +
  geom_col(data= covid_m, aes( x=mes, y= sum_m), color= "lightpink",width = 0.45,
           position = position_nudge(x = 0.225))
...
```{r}
ggplot() + geom_col( data= grafico_rios, aes(x = mes, y = ( b_mes)), color= "blue", width = 0.45,
           position = position_nudge(x = -0.225)) + 
  geom_col( data= grafico_rios, aes(x = mes, y = ( p_mes)), color= "green", width = 0.45,
           position = position_nudge(x = 0.225)) +
  ggtitle("Suma del caudal de los ríos Pandora y Bananito, por mes.") +
  xlab("Mes del año") +
  ylab("Suma de Caudal ") +
